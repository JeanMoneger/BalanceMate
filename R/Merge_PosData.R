...
Merge_PosData <- function(directory_path, SampleRate, SessionDuration = NULL) {
  ...
  if(!is.null(SessionDuration)) {
    if (length(problematic_files) > 0) {
      stop("The following file(s) have a different number of rows than expected (SampleRate * SessionDuration):\n",
           paste(problematic_files, collapse = ", "))
    }

    if (length(duration_problem_files) > 0) {
      stop("The following file(s) have a different duration than expected (SessionDuration):\n",
